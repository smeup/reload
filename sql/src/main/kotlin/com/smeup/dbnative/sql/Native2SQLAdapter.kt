...
class Native2SQL(val fileMetadata: FileMetadata) {
    ...
    fun getGenericSQL(): String {
        ...
        val sql =
            "SELECT " + columns.removeSuffix(", ") + " FROM " + fileMetadata.tableName + " ORDER BY " + fileMetadata.fileKeys.joinToString(
                separator = " " + SortOrder.ASCEDING.symbol + ", ",
                postfix = " " + SortOrder.ASCEDING.symbol
            )
        return sql
    }
}
